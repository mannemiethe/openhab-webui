...
async function getItemsWithTags() {
  ...
  const sourceResult = await api
    .getItems({ metadata: cfg.fetchMetadata, tags: cfg.itemTags, recursive: 'recursive' in cfg ? (cfg.recursive as boolean) : undefined })
    .catch((e) => {
...
async function getItemsInGroup() {
  ...
  const sourceResult = await api
    .getItemByName({
      itemName: cfg.groupItem,
      metadata: cfg.fetchMetadata,
      recursive: 'recursive' in cfg ? (cfg.recursive as boolean) : undefined
    })
    .catch((e) => {
      if (e instanceof ApiError && e.response.status === 404) {
