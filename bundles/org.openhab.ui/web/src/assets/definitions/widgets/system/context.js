...
export default () => [
  ...
  pt('constants', 'Widget Constants', 'Object with key:constant pairs. Constants are available to expressions in all child components via the <code>const</code> object.<br />Constants are evaluated before the widget is displayed and cannot be changed. If an initial item state dependency is not available yet, constants are re-evaluated once when the missing item state arrives.'),
